...
state: dict[str, Any] = json.load(open(STATE)) if os.path.exists(STATE) else {}
state.setdefault("sid", str(uuid.uuid4()))
SID: str = state["sid"]
H: dict[str, str] = {"X-Session-Id": SID}
ISSUES: list[str] = []
...
def ask(text: str, sid: str | None = None, start: str = START,
        wait: bool = True, quiet: bool = False) -> dict[str, Any]:
    h = {"X-Session-Id": sid or SID}
    t0 = time.time()
    r = HTTP.post(f"{BASE}/api/ask", json={"text": text, "start": start}, headers=h)
...
def current() -> str | None:
    return HTTP.get(f"{BASE}/api/current", headers=H).json()["current"]


def stage_hostile() -> None:
    print("\n== HOSTILE / BOUNDARY INPUTS ==")
    r = HTTP.post(f"{BASE}/api/ask", json={"text": "x" * 601, "start": START}, headers=H)
    check(r.status_code == 422, "601-char text rejected", f"HTTP {r.status_code}")
    r = HTTP.post(f"{BASE}/api/ask", json={"text": "hi"}, headers={"X-Session-Id": "../../etc"})
    check(r.status_code == 400, "bad session id rejected", f"HTTP {r.status_code}")
    r = HTTP.post(f"{BASE}/api/ask", json={"text": "hi"})
    ...
    for p in ["../.env", "output/usage.jsonl", "output/routes/../../.env"]:
        r = HTTP.post(f"{BASE}/api/current", json={"text": p}, headers=H)
        check(r.status_code == 400, f"/api/current refuses {p!r}", f"HTTP {r.status_code}")
    # uploads
    r = HTTP.post(f"{BASE}/api/upload", headers=H, files={"file": ("x.gpx", b"not a gpx at all")})
    check(r.status_code == 400, "garbage upload rejected", f"HTTP {r.status_code} {r.text[:80]}")
    r = HTTP.post(f"{BASE}/api/upload", headers=H, files={"file": ("x.gpx", b"")})
    ...
    r = HTTP.post(f"{BASE}/api/upload", headers=H, files={"file": ("x.gpx", one)})
    ...
    r = HTTP.post(f"{BASE}/api/upload", headers=H, files={"file": ("wild.gpx", wild)})
    ...
    r = HTTP.post(f"{BASE}/api/upload", headers=H, files={"file": ("nan.gpx", nan)})
    ...
    r = HTTP.post(f"{BASE}/api/upload", headers=H, files={"file": ("big.gpx", big)})
    check(r.status_code == 413, "oversize upload rejected", f"HTTP {r.status_code}")
    r = HTTP.post(f"{BASE}/api/undo", headers={"X-Session-Id": str(uuid.uuid4())})
...
def stage_route() -> None:
    ...
    if cands:
        r = HTTP.post(f"{BASE}/api/current", json={"text": cands[0]["gpx"]}, headers=other)
...
def stage_edits() -> None:
    ...
    r = HTTP.post(f"{BASE}/api/undo", headers=H).json()
    after = current()
...
def stage_upload() -> None:
    ...
    r = HTTP.post(f"{BASE}/api/upload", headers=h2,
